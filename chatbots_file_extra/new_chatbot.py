...
def init_fast_querier():
    ...
    try:
        ...
        # ⚡ CREATE INDEXES FIRST (CRITICAL!)
        logger.info("Creating/verifying Neo4j indexes...")
...
def query_knowledge_graph_fast(question: str, top_k: int = 15):
    ...
    try:
        ...
        for idx, item in enumerate(results):
            logger.debug(f"Processing result {idx}: {item}")
            entity = item.get('entity', '') or 'Unknown'
            label = item.get('label', '') or 'Entity'
            score = item.get('score', 0)
            neighbors = item.get('neighbors', []) or []
            
            # Add main entity - handle None values
            try:
                score_val = float(score) if score is not None else 0.0
                formatted_facts.append(
                    f"[Entity] {entity} ({label}) - relevance: {score_val:.2f}"
                )
            except (ValueError, TypeError) as e:
                logger.warning(f"Formatting error for entity {entity}: {e}")
                formatted_facts.append(f"[Entity] {entity} ({label})")
            ...
            for neighbor in neighbors[:3]:  # Top 3 neighbors per entity
                ...
                try:
                    ...
                    formatted_facts.append(
                        f"  → {entity} --[{rel_type}]--> {neighbor_text} (confidence: {conf_val:.2f})"
                    )
                except (ValueError, TypeError) as e:
                    logger.warning(f"Formatting error for relationship: {e}")
                    formatted_facts.append(
                        f"  → {entity} --[{rel_type}]--> {neighbor_text}"
                    )
        
        elapsed = time.time() - start_time
        logger.info(f"⚡ Query completed in {elapsed:.2f}s - Formatted {len(formatted_facts)} facts from {len(results)} entities")
...
def generate_answer(question: str, context: list):
    ...
    if not context:
        return "No relevant facts found in the knowledge graph. Please try rephrasing your question or ask about:\n- Supplier risk management\n- Risk assessment processes\n- Organizational relationships\n- Business processes\n- Compliance procedures"
...
"""
    ...
    try:
        ...
        # Fallback: Return the context directly with basic formatting
        fallback_response = f"I found the following information in the knowledge graph:\n\n"
        fallback_response += "\n".join([f"• {fact}" for fact in context[:10]])
        fallback_response += f"\n\n(Note: AI summarization unavailable - showing raw facts. Error: {str(e)[:100]})"
